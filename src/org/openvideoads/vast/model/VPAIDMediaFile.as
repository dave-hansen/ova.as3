...
package org.openvideoads.vast.model {
	...
	public class VPAIDMediaFile extends MediaFile implements VPAIDPlayback, FlashMedia {
		...
		public function VPAIDMediaFile() {
			super();
		}
		...
		public override function unload():void {
			CONFIG::debugging { doLog("Unloading the Linear VPAID resource...", Debuggable.DEBUG_VPAID); }
			if(isRunning()) {
				CONFIG::debugging { doLog("The VPAID Ad is still running sending stop() before unloading the resource", Debuggable.DEBUG_VPAID); }
				_adTerminated = true;
		...
		protected function startTimeoutTimer():void {
			if(_maxDurationTimeoutEnabled) {
				if(_timeoutTimer == null) {
					var seconds:int = ((_parentAd != null) ? ((_parentAd.getDurationAsInt() > 0) ? _parentAd.getDurationAsInt() : _maxDurationTimeoutInSeconds) : _maxDurationTimeoutInSeconds);
					if(seconds > 0) {
						if(_timeoutTimer != null) stopTimeoutTimer();
						_timeoutTimer = new Timer(seconds * 1000, 2);
					    _timeoutTimer.addEventListener(TimerEvent.TIMER, stopTimeoutTimer);
					    _timeoutTimer.start();	
						CONFIG::debugging { doLog("VPAIDMediaFile::startTimeoutTimer() - timer started - timeout set at " + seconds + " seconds", Debuggable.DEBUG_VPAID);	}			
					}
					else {
						CONFIG::debugging { doLog("VPAIDMediaFile::startTimeoutTimer() - cannot set timeout timer - no duration specified for this ad", Debuggable.DEBUG_VPAID);	}	
					}						
				} 
				else {
					CONFIG::debugging { doLog("VPAIDMediaFile::startTimeoutTimer() - ignoring request to set timeout time - timer is already active", Debuggable.DEBUG_VPAID);	}	
				}		
			}
			else {
				CONFIG::debugging { doLog("VPAIDMediaFile::startTimeoutTimer() - not setting a max duration timeout timer - disabled in config", Debuggable.DEBUG_VPAID); }
			}
		}
		
		public function stopTimeoutTimer(e:TimerEvent=null):void {
			if(_timeoutTimer != null) {
				CONFIG::debugging { doLog("VPAIDMediaFile::stopTimeoutTimer() - timer " + ((e == null) ? "aborted - no longer required" : "stopped - timeout triggered"), Debuggable.DEBUG_VPAID);	}
				_timeoutTimer.stop();
				_timeoutTimer = null;
				if(e != null) {
					try {
			...
			if(_parentAd != null) {
				if(_parentAd.parentAdContainer != null) {
					if(_parentAd.parentAdContainer.hasWrapper()) {
						_parentAd.parentAdContainer.wrapper.triggerTrackingEvent(eventType);
					}
					else _parentAd.parentAdContainer.triggerTrackingEvent(eventType);				
				}
			}
		}
		...
		protected function triggerClickTracking():void {
			CONFIG::debugging { doLog("VPAIDMediaFile::triggerClickTracking() - firing click tracking urls", Debuggable.DEBUG_VPAID); }
			if(_parentAd != null) {
			...
			try {
				...
				if(event.data != undefined && event.data != null) {
					if(event.data.playerHandles && !StringUtils.isEmpty(event.data.url)) {
						...
						CONFIG::debugging { doLog("VPAIDMediaFile::VPAID.AdClickThru event received - VPAID ad must be handling click through because data.playerHandles == " + event.data.playerHandles + " and data.url == " + event.data.url, Debuggable.DEBUG_VPAID);	}
					}		
				}
				else {
		...
		protected function onVPAIDAdUserClose(event:*):void {
			...
			triggerTrackingEvent(TrackingEvent.EVENT_CLOSE);		// VAST 1 & 2
			triggerTrackingEvent(TrackingEvent.EVENT_CLOSE_LINEAR); // VAST 3
